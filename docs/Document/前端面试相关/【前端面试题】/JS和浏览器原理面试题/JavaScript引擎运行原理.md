...
![image-20240523051834296](./JavaScript引擎运行原理.assets/image-20240523051834296.png)
...
![image-20240523051920521](./JavaScript引擎运行原理.assets/image-20240523051920521.png)

### 执行流程

![image-20240523051939932](./JavaScript引擎运行原理.assets/image-20240523051939932.png)

1. V8接收到要执行的JS 源代码
   - `源代码`对 `V8` 来说只是**一堆字符串**，`V8` 并不能直接理解这段字符串的含义

2. `V8`结构化这段字符串,生成了`{抽象语法树|AST}`,同时还会生成相关的**作用域**

3. 生成字节码(介于 `AST` 和 `机器代码` 的中间代码)
...
为了，更好的利用 CPU 资源，V8 采用**JIT**(Just In Time)技术提升效率:而是**混合编译执行和解释执行这两种手段**。
...
![image-20240523052048846](./JavaScript引擎运行原理.assets/image-20240523052048846.png)
...
![image-20240505235457574](./JavaScript引擎运行原理.assets/image-20240505235457574.png)
...
![image-20240505235539307](./JavaScript引擎运行原理.assets/image-20240505235539307.png)
...
![img](./JavaScript引擎运行原理.assets/v2-d16bd5820291747b599cbe93140fa584_720w.webp)
...
![动图](./JavaScript引擎运行原理.assets/v2-3baaa2030f404cf2375836958a3b2584_b.webp)
...
# JavaScript引擎垃圾回收机制|Garbage Collecation
...
1. 通过 `GC Root` 标记空间中活动对象和非活动对象
   ...
   - 浏览器环境中，`GC Root` 包括 1.**全局的 `window` 对象**,2.**文档 `DOM` 树**,由可以通过遍历文档到达的所有原生 `DOM` 节点组成,3.存放**栈上变量**
...
![image-20240523053426108](./JavaScript引擎运行原理.assets/image-20240523053426108.png)
...
2. 标记完成之后，就进入垃圾清理阶段,
   ...
   - ![img](./JavaScript引擎运行原理.assets/2ce7b356b6744d648896d.webp)
3. 完成复制后，**对象区域与空闲区域进行角色翻转**
   - ![img](./JavaScript引擎运行原理.assets/413e1b69fe.webp)
...
2. 垃圾的清除过程
   - `主垃圾回收器会直接将标记为垃圾的数据清理掉`
     ![image-20240523053557210](./JavaScript引擎运行原理.assets/image-20240523053557210.png)
...
![image-20240523040541560](./JavaScript引擎运行原理.assets/image-20240523040541560.png)
...
![image-20240318211223174](./JavaScript引擎运行原理.assets/image-20240318211223174.png)
...
JavaScript 引擎，如 V8，首先对 JavaScript 代码进行词法和语法分析，然后通过编译器将代码编译成可执行的机器码。引擎的结构包括堆（用于对象存储）和调用栈（用于方法调用和基本数据类型存储）。

## 什么是 V8 引擎，它在 Node.js 和浏览器中的作用是什么？

V8 引擎是 Google 开发的开源 JavaScript 引擎，它在 Chrome 浏览器和 Node.js 中被使用。V8 负责编译 JavaScript 代码到机器码，同时优化执行速度和内存使用。

## 什么是执行上下文（Execution Context）？

执行上下文是 JavaScript 代码被评估和执行时的环境。每当函数被调用时，一个新的执行上下文就会被创建并推入调用栈。执行上下文包括变量对象、作用域链和 this 指针。

## 单线程和事件循环如何协同工作？

JavaScript 是单线程的，意味着在同一时间只能执行一个任务。事件循环允许 JavaScript 引擎执行异步任务，通过回调队列和调用栈来协调同步和异步代码的执行。
...
![image-20240318213535703](./JavaScript引擎运行原理.assets/image-20240318213535703.png)
...
![image-20240318213617239](./JavaScript引擎运行原理.assets/image-20240318213617239.png)
...
![image-20240318213658629](./JavaScript引擎运行原理.assets/image-20240318213658629.png)
...
![image-20240318211730347](./JavaScript引擎运行原理.assets/image-20240318211730347.png)
...
![image-20240318211754772](./JavaScript引擎运行原理.assets/image-20240318211754772.png)
...
![image-20240318213228496](./JavaScript引擎运行原理.assets/image-20240318213228496.png)
...
![image-20240318213246674](./JavaScript引擎运行原理.assets/image-20240318213246674.png)
...
![image-20240318213404633](./JavaScript引擎运行原理.assets/image-20240318213404633.png)
...
JavaScript 使用自动垃圾回收机制来管理内存。最常见的方法是标记清除（Mark-and-Sweep），引擎会定期查找那些不再被应用程序引用的对象，并释放它们占用的内存。
...
[深入理解 V8 的垃圾回收原理 - 简书 (jianshu.com)](https://www.jianshu.com/p/b8ed21e8a4fb)

[JavaScript 中的垃圾回收 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/23992332)



## JavaScript 引擎如何处理异步回调？

当 JavaScript 代码执行异步操作（如 AJAX 请求）时，回调函数会被放入事件队列。一旦调用栈为空，事件循环会将事件队列中的回调函数推入调用栈执行。
...
![image-20240326224426235](./JavaScript引擎运行原理.assets/image-20240326224426235.png)
...
HTML5 标准规定了 setTimeout()的第二个参数的最小值（最短间隔），不得低于**4 毫秒**，如果低于这个值，就会自动增加。在此之前，老版本的浏览器都将最短间隔设为 10 毫秒。另外，对于那些 DOM 的变动（尤其是涉及页面重新渲染的部分），通常不会立即执行，而是**每 16 毫秒执行一次**。这时使用 requestAnimationFrame()的效果要好于 setTimeout()。
...
for 循环是同步任务，会先执行完循环，此时 i 的值是 4。4ms 后 `console.log(i)` 被依次放入任务队列，此时如果执行栈中没有同步任务了，就从任务队列中依次取出任务，所以打印出 4 个 4。
...
- 14.没有微任务，第二次 Event Loop 结束； 
- 15.取出 timer4 执行，console.log(6)同步任务，直接执行，打印 6； 
- 16.没有微任务，第三次 Event Loop 结束； 
- 17.取出 timer3 执行，console.log(5)同步任务，直接执行，打印 5； 
- 18.没有微任务，第四次 Event Loop 结束； 
- 19.取出 timer1 执行，console.log(2)同步任务，直接执行，打印 2； 
- 20.没有微任务，也没有宏任务，第五次 Event Loop 结束； 
...
- 5.timer1 里面的 setTimeout 是异步任务且宏函数，记做 timer3 放到宏函数队列； 
- 6.没有微任务，第二次 Event Loop 结束； 
- 7.取出 timer2，console.log(3)同步任务，直接执行，打印 3； 
- 8.没有微任务，第三次 Event Loop 结束； 
- 9.取出 timer3，console.log(2)同步任务，直接执行，打印 2； 
- 10.没有微任务，也没有宏任务，第四次 Event Loop 结束； 
- 11.结果：1，3，2
...
**JS 中的垃圾回收机制**： JavaScript 的垃圾回收机制主要依赖于两种策略：标记-清除（Mark-and-Sweep）和分代收集（Generational Collection）。
...
```c
bool keep`running = true;
void MainTherad(){
    for(;;){
        //执行队列中的任务
        Task task = task`queue.takeTask();
        ProcessTask(task);
        //执行延迟队列中的任务
        ProcessDelayTask()
        if(!keep`running) // 如果设置了退出标志，那么直接退出线程循环
            break;
    }
}
```
...
![任务队列和Event Loop](./JavaScript 引擎运行原理.assets/任务队列和 Event Loop.jpg)
